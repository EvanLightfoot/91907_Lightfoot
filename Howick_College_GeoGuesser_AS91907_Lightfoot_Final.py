...
class Game:
    ...
    def get_guess(self, root):
        ...
        if self.guess == '': # If the user didn't enter anything but submitted their guess, do not proceed.
            ...
            self.clear_labels(root)
            output_text = Label(output_msg_frame, text='Answers are 13 letters max!', bg='white', fg='darkred', font=('Calibri', '20', 'bold'))
...
title_banner_strip = Frame(root, bg='#d3b846', height=5)
title_banner_strip.pack(fill='x')
title_banner = Frame(root, bg='#08345c', height=10)
title_banner.pack(fill='x')
title = Label(title_banner, text='Howick College GeoGuesser 2024', bg='#08345c', fg='#d3b846', font=('Calibri', '30', 'bold'))
title.pack(pady=5)
output_msg_frame = Frame(root, bg='white', height=40, width=450, highlightcolor='black', highlightthickness=3)
output_msg_frame.pack(anchor='n', pady=(10, 0))
output_msg_frame.propagate(False)
...
guess_text_box = Entry(root, textvariable=guess, width=35, highlightbackground='black', highlightthickness='1', font=('Calibiri', '17'))
...
submit_btn = Button(button_frame, text='Submit', borderwidth='2', width='13', bg='#32CD32', fg='black', font=('Calibri', '15', 'bold'),command=lambda:instance.start_game(root))
...
crest_frame = Frame(root, bg='white')
crest_frame.pack(side='bottom', anchor='sw')
crest_img_label = Label(crest_frame, image=crest_img, bg='white')
crest_img_label.pack()
